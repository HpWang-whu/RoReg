import torch
import utils.utils as utils
from tqdm import tqdm
import numpy as np
from train.loss import name2loss

# validation of group feature extraction
class Validation_GF:
    def __init__(self,cfg):
        self.cfg=cfg
        self.loss=name2loss[self.cfg.loss_type](cfg)
    
    def recall(self,data):
        feats0=data["feats0_inv"]
        feats1=data['feats1_inv']
        bn,fn=feats0.shape
        scores=torch.norm(feats0[None,:,:]-feats1[:,None,:],dim=-1)
        idxs_pr=torch.argmin(scores,1)
        idxs_gt=torch.arange(bn).to(feats0.device).long()
        correct_rate=torch.mean((idxs_pr==idxs_gt).float())
        return correct_rate
    
    def recall_index(self,data):
        feats0=data["feats0_inv"]
        feats1=data['feats1_inv']
        bn,fn=feats0.shape
        scores=torch.norm(feats0[None,:,:]-feats1[:,None,:],dim=-1)
        idxs_pr=torch.argmin(scores,1)
        idxs_gt=torch.arange(bn).to(feats0.device).long()
        return torch.where(idxs_gt==idxs_pr)[0]

    def __call__(self, model, eval_dataset):
        model.eval()
        eval_results={}
        alloutput0=[]
        alloutput1=[]
        allloss=[]
        all_batch_recall=[]
        all_DR_ability=[]
        all_DR_ok=[]
        
        for data in tqdm(eval_dataset):
            data = utils.to_cuda(data)
            with torch.no_grad():
                outputs=model(data)
                #for whole cal
                alloutput0.append(outputs['feats0_inv'].cpu())
                alloutput1.append(outputs['feats1_inv'].cpu())
                all_DR_ability.append(outputs['DR_pre_ability'].cpu())
                all_DR_ok.append((outputs['DR_true_index']==outputs['DR_pre_index']).cpu().numpy())
                #val loss
                loss=self.loss(outputs, data)
                allloss.append(loss)
                #batch val
                bc_recall=self.recall(outputs)
                all_batch_recall.append(bc_recall)

        #val loss
        val_loss=torch.mean(torch.tensor(allloss))
        #batch_recall
        batch_recall=torch.mean(torch.tensor(all_batch_recall))
        #whole dataset recall
        alloutput0=torch.cat(alloutput0,dim=0)
        alloutput1=torch.cat(alloutput1,dim=0)
        
        alloutputs={'feats0_inv':alloutput0,'feats1_inv':alloutput1}
        whole_recall=self.recall(alloutputs)
        ok_index=self.recall_index(alloutputs).cpu().numpy().astype(np.int)
        all_DR_ok=np.concatenate(all_DR_ok)
        double_ok_rate=np.mean(all_DR_ok[ok_index])

        return {"val_loss":val_loss, "whole_recall":whole_recall, 'batch_recall':batch_recall,'PartI_DR_ability':double_ok_rate}

# validation of equivariance guided rotation estimaiton
class Validation_ET:
    def __init__(self,cfg):
        self.cfg=cfg
        self.loss=name2loss[self.cfg.loss_type](self.cfg)
        
    def diff_cal(self,R_pre,R_gt):
        result=0
        eps=1e-7
        result=[]
        R_pre=R_pre/torch.clamp_min(torch.norm(R_pre,dim=1,keepdim=True),min=1e-4)#q
        for i in range(R_pre.shape[0]):
            loss_q = torch.clamp_min((1.0 - torch.sum(R_pre[i]*R_gt[i])**2),min=eps)
            err_q = torch.acos(1 - 2 * loss_q)
            result.append(err_q/np.pi*180)
        return result
    
    def static(self,errors):
        result=torch.zeros(6)
        for e in errors:
            e_index=int(e)
            if e_index<6:
                result[e_index]+=1
        result/=errors.shape[0]
        return result

    def __call__(self, model, eval_dataset):
        model.eval()
        eval_results={}
        alloutput0=[]
        part1_ability=[]
        all_loss=[]
        all_R_error=[]
        
        for data in tqdm(eval_dataset):
            quaternion_gt=torch.squeeze(data['deltaR'])
            data = utils.to_cuda(data)
            with torch.no_grad():
                outputs=model(data)
                quaternion=outputs['quaternion_pre'].cpu()
                part1_ability.append(outputs['part1_ability'])
                #val loss
                loss=self.loss(outputs, data).cpu()
                all_loss.append(loss)
                #batch val
                ok_idxs=torch.where(outputs['pre_idxs'].cpu()==outputs['true_idxs'].cpu())
                R_error=self.diff_cal(quaternion,quaternion_gt)
                for i in range(len(R_error)):
                    all_R_error.append(R_error[i])
                
        #val loss
        all_loss=torch.Tensor(all_loss)
        all_R_error=torch.Tensor(all_R_error)
        part1_ability=torch.Tensor(part1_ability)
        R_error_statics=self.static(all_R_error)
        
        return {'val_loss':torch.mean(all_loss), 'R_error':torch.mean(all_R_error),'part1_ability':torch.mean(part1_ability),'R_error_statics':R_error_statics}

# validation of rotation guided detector
class Validation_RD:
    def __init__(self,cfg):
        self.cfg=cfg
        self.loss=name2loss[self.cfg.loss_type](self.cfg)

    def __call__(self, model, eval_dataset):
        model.eval()
        all_loss=[]
        all_okrate=[]
        all_avgRdiffs=[]
        oknum=0
        basenum=0
        for data in tqdm(eval_dataset):
            data = utils.to_cuda(data)
            # model(data)
            with torch.no_grad():
                # print('eval')
                outputs=model(data)
                #val loss
                loss=self.loss(outputs,data)
                all_loss.append(loss)
                #batch val
                scores=outputs['scores'].cpu().numpy()
                Rdiffs=outputs['Rdiffs'].cpu().numpy()
                bs=Rdiffs.shape[0]
                scoresA=scores[0:bs]
                scoresB=scores[bs:]
                scores=scoresA+scoresB
                okscore=np.mean(scores[Rdiffs<15/60])
                falsescore=np.mean(scores[Rdiffs>15/60])
                if np.max(Rdiffs)<15/60:
                    falsescore=2
                if np.min(Rdiffs)>15/60:
                    okscore=0
                all_okrate.append((okscore-falsescore)/falsescore)

                #Rdiff about:
                scores=scores/np.sum(scores)
                all_avgRdiffs.append(np.mean(scores*Rdiffs)*60)
        
        #val loss
        all_loss=torch.Tensor(all_loss)
        all_okrate=torch.Tensor(all_okrate)    
        all_avgRdiffs=torch.Tensor(all_avgRdiffs)
        print('okRdiffs:',torch.mean(all_avgRdiffs))
        return {'val_loss':torch.mean(all_loss), 'val_recall':torch.mean(all_okrate), 'val_avgRdiffs':torch.mean(all_avgRdiffs)}


# validation of rotation coherence detector
class Validation_RM:
    def __init__(self,cfg):
        self.cfg=cfg
        self.loss=name2loss[self.cfg.loss_type](self.cfg)

    def __call__(self, model, eval_dataset):
        model.eval()
        all_loss=[]
        all_recall=[]
        oknum=0
        basenum=0
        for data in tqdm(eval_dataset):
            trueindex=data['true_idx'].numpy()[0,0]
            data = utils.to_cuda(data)
            # model(data)
            with torch.no_grad():
                # print('eval')
                outputs=model(data)
                #val loss
                loss=self.loss(outputs,data)
                all_loss.append(loss)
                #batch val
                pair_pre=outputs['matches0'].cpu().numpy()
                pair_true=data['matches0'].cpu().numpy()
                all_recall.append(np.mean(pair_pre==pair_true))
                for i in range(pair_pre.shape[0]):
                    for j in range(pair_pre.shape[1]):
                        if not pair_true[i,j]==-1:
                            basenum+=1
                            if pair_true[i,j]==pair_pre[i,j]:
                                oknum+=1
        #val loss
        all_loss=torch.Tensor(all_loss)
        all_recall=torch.Tensor(all_recall)
        print('val_recall:',torch.mean(all_recall))
        return {'val_loss':torch.mean(all_loss), 'val_recall':torch.mean(all_recall),'pair_ok_rate':oknum/basenum}#,'coarse success rate':successrate}


name2val={
    "val_gf":Validation_GF,
    "val_rd":Validation_RD,
    "val_rm":Validation_RM,
    'val_et':Validation_ET
}
